...
@testset "show RiFID" begin
    io = IOBuffer()
    show(io, RiFID(Int64[1]u"s", "Kovats", Float64[1000], Int64[12]))
    @test String(take!(io)) == string(
        "JuChrom.RiFID {scan times: Int64, retention indices: Float64, intensities: Int64}\n",
        "1 scan; scan time: 1 s\n",
        "1 retention index: 1000.0 (≘ 1 s)\n",
        "intensity: 12\n",
        "metadata: 0 entries")
    show(io, RiFID(Int64[1, 2]u"s", "Kovats", Float64[1000, 2000], Int64[12, 13]))
    @test String(take!(io)) == string(
        "JuChrom.RiFID {scan times: Int64, retention indices: Float64, intensities: Int64}\n",
        ...
        "metadata: 0 entries")
    show(io, RiFID(convert(Vector{Int64}, (1:10))u"s", "Kovats", 1000:100.0:1900, 
    ...
    @test String(take!(io)) == string(
        ...
        "metadata: 0 entries")
    show(io, RiFID(convert(Vector{Int64}, (1:11))u"s", "Kovats", 1000:100.0:2000, 
    ...
    @test String(take!(io)) == string(
        ...
        "metadata: 0 entries")
    show(io, RiFID(Int64[1, 2, 3]u"s", "Kovats", [missing, 2000, 3000], Int64[12, 956, 23], 
        Dict(:id => 4)))
    @test String(take!(io)) == string(
        "JuChrom.RiFID {scan times: Int64, retention indices: Union{Missing, Int64}, ",
        "intensities: Int64}\n",
        "3 scans; scan times: 1 s, 2 s, 3 s\n",
        "2 retention indices: 2000 (≘ 2 s), 3000 (≘ 3 s)\n",
        "intensity range: 12 - 956\n",
        "metadata: 1 entry")
    show(io, RiFID(Int64[1, 2, 3]u"s", "Kovats", [missing, 2000, missing], 
