...
@testset "importdata ANDI()" begin
    ...
    gcms = importdata(file, ANDI(; scantimetype=Float32))
    io = IOBuffer()
    Base.show(io, gcms)
    @test String(take!(io)) == string(
    ...
    gcms = importdata(file, ANDI(; iontype=Float64))
    io = IOBuffer()
    Base.show(io, gcms)
    @test String(take!(io)) == string(
    ...
    gcms = importdata(file, ANDI(; intensitytype=Float64))
    io = IOBuffer()
    Base.show(io, gcms)
    @test String(take!(io)) == string(
    ...
    gcms = importdata(file, ANDI(; scantimetype=Float32, iontype=Float64, 
