...
function abundance_test_channelinfo()
    (
        references=[
            (carbon=8, mzindices=[1, 3], referenceintensities=[2.0, 1.0]),
            (carbon=9, mzindices=[2], referenceintensities=[4.0]),
        ],
    )
end

@testset "alkaneabundance weighted least-squares estimate" begin
    X, variance, mzindices, referenceintensities = abundance_test_inputs()

    abundance = JuChrom.alkaneabundance(
        X,
        variance,
        mzindices,
    ...
    floored = JuChrom.alkaneabundance(
        X,
        variance,
        mzindices,
        referenceintensities;
    ...
    clipped = JuChrom.alkaneabundance(
        X,
        variance,
        mzindices,
        referenceintensities;
...
@testset "alkaneabundancevariance formula" begin
    _, variance, mzindices, referenceintensities = abundance_test_inputs()

    abundancevariance = JuChrom.alkaneabundancevariance(
        variance,
        mzindices,
        referenceintensities;
        variancefloor=1.0,
    ...
    floored = JuChrom.alkaneabundancevariance(
        variance,
        mzindices,
        referenceintensities;
        variancefloor=4.0,
...
@testset "alkaneabundances and alkaneabundancevariances wrappers" begin
    X, variance, mzindices, referenceintensities = abundance_test_inputs()
    msm = MassScanMatrix([1.0, 2.0, 3.0], [29.0, 43.0, 57.0, 71.0], X)
    channelinfo = abundance_test_channelinfo()

    abundances = JuChrom.alkaneabundances(
        msm,
        variance,
        channelinfo;
        variancefloor=1.0,
    ...
    abundancevariances = JuChrom.alkaneabundancevariances(
        variance,
        channelinfo;
        variancefloor=1.0,
    )
    ...
    @test abundances[8] ≈ JuChrom.alkaneabundance(
        X,
        variance,
        mzindices,
        referenceintensities;
    ...
    @test_throws DimensionMismatch JuChrom.alkaneabundances(
        msm,
        ones(2, 4),
        channelinfo,
    )
...
@testset "alkaneabundanceinfo combines abundance pipeline" begin
    X, variance, _, _ = abundance_test_inputs()
    msm = MassScanMatrix([1.0, 2.0, 3.0], [29.0, 43.0, 57.0, 71.0], X)
    channelinfo = abundance_test_channelinfo()

    abundanceinfo = JuChrom.alkaneabundanceinfo(
        msm,
        variance,
        channelinfo;
        variancefloor=4.0,
...
@testset "alkane reference abundance intensity validation" begin
    reference = (carbon=8, referenceintensities=Float32[1.0, 0.5])
    ...
    @test_throws ArgumentError JuChrom.alkane_reference_abundance_intensities(
        (carbon=8, referenceintensities=[1.0, Inf]),
    )
    @test_throws ArgumentError JuChrom.alkane_reference_abundance_intensities(
        (carbon=8, referenceintensities=[1.0, -0.5]),
    )
end
...
@testset "abundance input validation helpers" begin
    ...
    @test_throws ArgumentError JuChrom.validate_alkane_abundance_variances_matrix([1.0, 2.0])
    ...
    @test_throws DimensionMismatch JuChrom.alkaneabundance(
        X,
        variance[:, 1:3],
        [1, 3],
        referenceintensities;
...
@testset "alkane abundance window validation helpers" begin
    ...
    @test_throws ArgumentError JuChrom.alkane_abundance_values(["bad"], 8)
...
@testset "alkaneabundancewindows peak boundaries" begin
    ...
    variance_limited = JuChrom.alkaneabundancewindows(
        Dict(8 => abundance),
    ...
    @test_throws ArgumentError JuChrom.alkaneabundancewindows(
        Dict(8 => abundance),
        [0.1, 0.2],
    )
end
...
@testset "alkane abundance peak-window helpers" begin
    ...
    leftindex, leftstop = JuChrom.alkane_abundance_peak_window_left(
        abundance,
        nothing,
        minima,
        maxima,
    ...
    rightindex, rightstop = JuChrom.alkane_abundance_peak_window_right(
        abundance,
        nothing,
        minima,
        maxima,
    ...
    @test JuChrom.alkane_abundance_peak_window_left(
        [1.0, 2.0, 1.0],
        nothing,
        Set{Int}(),
        [2],
    ...
    @test JuChrom.alkane_abundance_peak_window_right(
        [1.0, 2.0, 1.0],
        nothing,
        Set{Int}(),
        [2],
